...
Consigna: Las ultimas 3 cifras de mi DNI son 175,crear una variable que tenga ese numero

```{r}
DNI=175
```

Consigna: Crear un vector que tenga todos los numeros enteros desde el 1 al 175
...
ggplot(df, aes(x = cluster, y = vibracion, fill = cluster)) +
  ...
  theme_minimal()
```
## Ejercicio 7: Matrices e Inversa

En este ejercicio, crearemos una matriz cuadrada de 3x3 utilizando el valor de tu DNI (175) y otros valores para asegurar que la matriz sea invertible (que su determinante sea distinto de cero). Luego, utilizaremos la función solve que es la estándar en R para obtener la matriz inversa.
